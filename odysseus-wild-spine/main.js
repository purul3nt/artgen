...
(async () => {
  ...
  const SYMBOLS = {
    wild: {
      notes: { idle: "breathing, head bob, banner glint · loop", land: "drop, squash, roar, settle → idle", connect: "belly laugh, gold swirl, sparkles" },
    },
    bonus: {
      notes: { idle: "breathing, goblet swirl, twinkles · loop", land: "drop, squash, giggle, wine splash → idle", connect: "“Cheers!” toast, double splash, pink swirls" },
    },
  };
  const SKELETON_SIZE = 520; // matches skeleton width/height in the JSON
  ...
  await Assets.load(Object.keys(SYMBOLS).flatMap((key) => [`${key}Skeleton`, `${key}Atlas`]));

  let symbol = location.hash.slice(1) in SYMBOLS ? location.hash.slice(1) : "wild";
  let hero = null;
  let cells = [];
  ...
  function layout() {
    ...
    hero.scale.set(Math.min(w, h) / (SKELETON_SIZE * 1.12));
    hero.position.set(w / 2, h / 2 - 6);
    const cell = Math.min(w, h - 30) / 3;
    cells.forEach((s, i) => {
      s.scale.set(cell / (SKELETON_SIZE * 0.86));
  ...
  speed.addEventListener("input", () => [hero, ...cells].forEach((s) => { s.state.timeScale = Number(speed.value); }));
  ...
  window.addEventListener("keydown", (e) => {
    const name = { 1: "idle", 2: "land", 3: "connect" }[e.key];
    if (name) select(name);
  });
